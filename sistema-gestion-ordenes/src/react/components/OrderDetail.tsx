...
export default function OrderDetail({ orderId, onClose }: OrderDetailProps) {
  ...
  // Regla simple y explícita: si un servicio aparece en equipos adicionales,
  // no se muestra en Equipo 1 (evita acumulación visual en órdenes antiguas).
  const additionalServiceNames = new Set<string>();
  additionalDevicesWithAllocatedServices.forEach((device: any) => {
    const services = Array.isArray(device.selected_services) ? device.selected_services : [];
    services.forEach((service: any) => {
      additionalServiceNames.add(String(service?.name || service?.service_name || "").trim().toLowerCase());
    });
  });

  if (additionalServiceNames.size > 0) {
    const filteredByName = firstDeviceServices.filter(
      (service) => !additionalServiceNames.has(String(service.name || "").trim().toLowerCase())
    );

    // Si al filtrar por nombre queda vacío, mantenemos la versión previa para no ocultar todo.
    if (filteredByName.length > 0) {
      firstDeviceServices = filteredByName;
    }
  }
