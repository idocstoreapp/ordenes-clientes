...
export default function DeviceCatalogSettings() {
  ...
  const visibleItems = useMemo(
    () => items.filter((item) => item.level === activeLevel),
    [items, activeLevel]
  );
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
        Aquí puedes administrar tipos, marcas, líneas, modelos y variantes con sus imágenes/logos.
      </div>
        ...
        {visibleItems.map((item) => (
          <div key={item.id} className="rounded-lg border border-slate-200 bg-white p-3">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              {item.image_url && <img src={item.image_url} alt={item.label} className="h-20 w-full rounded-md object-cover border border-slate-200" />}
              {item.logo_url && <img src={item.logo_url} alt={`${item.label} logo`} className="h-20 w-full rounded-md object-contain border border-slate-200 bg-white" />}
            </div>
          </div>
        ))}
